...
#              9 尝试写一下指套交互的代码，在8的基础上 已完成的手指交互
...
def angle_between_vectors(a, b):
    # 计算两个点的点积用np.dot,计算数组和一个点的点积用np.matmul
    ...
    try:
        while True:
            ...
            if eyetohand_camera_status:
                ...
                arucomarker_image, corners_set, matrix_set = Aruco_marker(img_color_1)   # 返回的是带aruco的图像和激光笔在相机中的姿态矩阵
                ...
                if len(matrix_set) > 0:
                    ...
                    # error_matrix = np.array([[0.98366, -0.17948, -0.013993, 0.049077],    # 标定板对齐的补偿矩阵
                    #                          [0.18003, 0.9806, 0.077612, -0.039515],
                    #                          [-0.00020862, -0.078863, 0.99689, 0.0056676],
                    #                          [0, 0, 0, 1]])
                    error_matrix = np.array([[0.98013, -0.19026, -0.056146, 0.068862],  # colored ICP匹配的补偿矩阵
                                             [0.19498, 0.9761, 0.095999, -0.055916],
                                             [0.03654, -0.10504, 0.9938, 0.014556],
                                             [0, 0, 0, 1]])
                    # error_matrix = np.eye(4)    # 无补偿矩阵
            ...
            if eyeonhand_camera_status:
                ...
                # print("point6:", point6)
                # o3d.visualization.draw([point6])
                point7 = point6.uniform_down_sample(10)
                # print("point7:", point7)
                # o3d.visualization.draw([point7])
                pcl_cloud_set = np.asarray(point7.points)
                if len(laser_pointer_start) > 0:
                    vector7_set = pcl_cloud_set - laser_pointer_start
                    # print("vector6_set:", vector7_set)
                    angle_set = angle_between_vectors(vector7_set, laser_pointer_direction)
                    angle_index = np.argmin(angle_set)
                    finger_points = pcl_cloud_set[angle_index]
                    color_pixel = rs.rs2_project_point_to_pixel(color_intrin_2, finger_points)
                    print("color_pixel", color_pixel)
                    cv2.circle(img_color_2, (int(color_pixel[0]), int(color_pixel[1])), 8, (0, 255, 0), -1)
            else:
                img_color_2 = np.full((resolution_y, resolution_x, 3), 255, dtype=np.uint8)
                text_2 = "Waiting for eye-on-hand camera connection"
                cv2.putText(img_color_2, text_2, (20, int(resolution_y/2)),
                            cv2.FONT_HERSHEY_SCRIPT_SIMPLEX,
                            1, (0, 0, 0), 2)
            ...
            # axis_all_2 = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2, origin=laser_pointer_start)
            # axis_all_2.rotate(mean_matrix[:3, :3], center=laser_pointer_start)
            # o3d.visualization.draw_geometries([point_111, point_222, axis_all_1, axis_all_2])
            current_time = time.time()
            fps = 1 / (current_time - pre_time)
            print("fps", fps)
            cv2.imshow("img_color_1", np.hstack((arucomarker_image, img_color_2)))  # 展示彩色图像和深度图像
