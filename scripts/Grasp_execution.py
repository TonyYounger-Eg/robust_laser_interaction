...
def grasp_callback(grasp_info):
    ...
    # touch_center_after_eyeonhand = np.matmul(eye_on_hand_matrix, touch_center) + np.array([0, 0, 0 + 0.05, 0])   # Z轴增加5cm抓的更牢固
    touch_center_after_eyeonhand = np.matmul(eye_on_hand_matrix, touch_center) + np.array([0, 0, 0, 0])
